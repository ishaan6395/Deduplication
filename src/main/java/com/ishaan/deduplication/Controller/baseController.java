...
public class baseController {
    ...
    public ModelAndView index(){
        ...
        BufferedReader br;
        try{
            f = new File("C:\\Users\\thakk\\Downloads\\takehome\\normal.csv");
            fr = new FileReader(f);
            br = new BufferedReader(fr);
           
            HashMap<Integer, List<UserData>> map = new HashMap<>();
    ...
    public static boolean isSame(UserData user1, UserData user2){
        ...
        if(user1_email.equals(user2_email) || user1_phone.equals(user2_phone)){
            ...
            if(ld.apply(user1_lastName, user2_lastName)<2 || 
                ...
                if(ld.apply(user1_company, user2_company) <
                        Math.max(user1_company.length(),user2_company.length())*0.65
                        ||
                        ld.apply(user1_address1,user2_address1) < 
                        Math.max(user1_address1.length(),user2_address1.length())*0.65){
